function [B] =BRG(Pre,Post,M0,Te2)
...
no=size(Te2,2);    %%%%%no赋值为Te的cardinality,即explicit变迁的个数!
% for i=1:sizeL
...
for i=1:no %%%%%%%%%no为explicit变迁的个数
    ...
    t=Te2(i); %%%%%Te中的第i列元素赋值于t,也就是第i个explciit变迁
...
%				T{n,6}(1,i)=1;
                ...
                for jj=1:sizeJi
                    ...
                    B{n,6}{count,i}={[Te2(i)] [numbers(transition)] B{n,4}{1,transition}{jj,1}};
                end
			end
%		end
	end
